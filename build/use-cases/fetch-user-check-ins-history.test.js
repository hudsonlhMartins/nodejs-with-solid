...
// src/use-cases/fetch-user-check-ins-history.test.ts
describe("FetchUserCheckInsUseCase", () => {
  let checkInsRepository;
  let sut;
  beforeEach(() => {
  ...
  it("should be able to fetch user check ins", async () => {
    ...
    globalExpect(checkIns).toHaveLength(1);
    globalExpect(checkIns).toEqual([checkIn]);
  });
  it("should be able to fetch paginated check-in check ins", async () => {
    ...
    globalExpect(checkIns).toEqual([
      globalExpect.objectContaining({ gym_id: "gym-id_20" }),
      globalExpect.objectContaining({ gym_id: "gym-id_21" })
    ]);
  });
});
